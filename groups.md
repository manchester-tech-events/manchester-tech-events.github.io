...
{% assign group_name = group_hash[0] | replace: '_', ' ' %}
    <tr>
      ...
      <td>{% if group.twitter %}<a href='https://twitter.com/{{ group.twitter }}' target='_new'><i class="circular twitter icon"></i></a>{% endif %}{% if group.social %}<a href='https://{{ group.social }}' target='_new'><i class="circular at icon"></i></a>{% endif %}</td>
